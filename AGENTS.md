...
- Keep the TUI as the only participant client. The internal moderation command
  is operator-only; the separate landing site only explains the project and
...
- Treat letter text, device proofs, access and revocation credentials, invite
  codes, KMS data, Access JWTs, and forwarded network data as hostile input.
  Never log plaintext, authentication material, ciphertext, or key material.
- Keep database state transitions in short PostgreSQL transactions. The server
  remains authoritative. Never hold a transaction or row lock across a KMS call.
- Keep device-key, DPoP, envelope-context, key-ARN, and moderation-audit checks
  fail-closed. Do not add message or evidence plaintext persistence or an
  evidence-decrypt credential to the Railway runtime.
...
Test observable behavior, authorization boundaries, replay resistance,
encryption failure, state transitions, input limits, concurrency, and
generated-code drift. Do not snapshot terminal styling.
